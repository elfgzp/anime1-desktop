...
class PeeweeMigrationHelper:
    ...
    def _try_migrate_with_backup(self, version: int) -> bool:
        ...
        try:
            ...
            if self._migrate_data_from_backup(backup_path):
    ...
    def _migrate_data_from_backup(self, backup_path: Path) -> bool:
        """Migrate data from backup database.

        This preserves favorites and playback_history, but may lose cover_cache.
        """
        from peewee import SqliteDatabase
        ...
        try:
            ...
            # Migrate cover_cache (without the problematic column)
            cursor = backup_db.execute_sql("SELECT anime_id, cover_data, cached_at FROM cover_cache")
            for row in cursor.fetchall():
                try:
                    self.db.execute_sql(
                    ...
                    # If the above fails due to schema mismatch, try without cached_at
                    try:
                        self.db.execute_sql(
                            "INSERT OR IGNORE INTO cover_cache (anime_id, cover_data) VALUES (?, ?)",
                            (row[0], row[1])
                        )
                    except:
                        pass
