  ...
  try {
    ...
    if (animeList.value.length > 0) {
      ...
      normalAnime.forEach(anime => {
        // 启动后台加载封面
        animeAPI.getCover(anime.title).then(response => {
          if (response.data && response.data.coverUrl) {
            // 使用 nextTick 确保骨架屏有足够时间显示
            nextTick(() => {
              anime.cover_url = response.data.coverUrl
...
const checkFavoritesStatus = async () => {
  ...
  try {
    ...
    if (response.success && response.data) {
      // response.data is a boolean array
      const statusMap = {}
      animeList.value.forEach((anime, index) => {
        statusMap[anime.id] = response.data[index] || false
      })
      isFavoriteMap.value = statusMap
    }
  } catch (error) {
    ...
    const promises = animeList.value.map(async (anime) => {
      try {
        const isFav = await favoriteAPI.isFavorite(anime.id)
        isFavoriteMap.value[anime.id] = isFav
      } catch {
