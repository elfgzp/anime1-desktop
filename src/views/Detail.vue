...
const loadEpisodeProgress = async () => {
  ...
  try {
    ...
    console.log('[Detail] 加载播放进度响应:', response)

    if (response.success && response.data) {
      // response.data is already a map
      episodeProgressMap.value = response.data
...
const loadBackgroundData = () => {
  ...
  return Promise.all([
    ...
    (async () => {
      ...
      try {
        isFavorite.value = await favoriteAPI.isFavorite(animeId.value)
...
const playEpisode = async (idx, retryCount = 0) => {
  ...
  try {
    // 获取视频 URL - 使用 IPC 调用主进程的视频代理服务
    console.log('[Video] 请求视频信息:', ep.url)
    const response = await animeAPI.getVideoInfo(ep.url)
    console.log('[Video] IPC 响应:', response)

    // IPC returns { success: true, url, cookies }
    const data = response.success ? response : { error: response.error || 'Unknown error' }

    if (!response.success || data.error) {
      console.log('[Video] API返回错误:', data.error)
      // 如果是临时错误（视频还在处理中），重试
    ...
    // 在 Electron 中，我们直接使用原始视频 URL
    // 因为 Electron 的 webview 可以处理跨域请求
...
const fetchBangumiInfo = async () => {
  if (!animeId.value || !animeData.value?.anime?.title) {
    return
  }

  try {
    const response = await animeAPI.getBangumiInfo(animeId.value, animeData.value.anime.title)
