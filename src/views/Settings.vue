...
const handleCheckUpdate = async () => {
  ...
  try {
    ...
    // 检查是否是更新检查失败（如速率限制）
    if (data && data.error_type === 'update_check_failed') {
    ...
    if (data && data[RESPONSE_FIELDS.HAS_UPDATE]) {
      // 保存更新信息，显示下载按钮
      updateInfo.value = {
        has_update: true,
...
const loadAbout = async () => {
  try {
    ...
    if (response[RESPONSE_FIELDS.SUCCESS] && response[RESPONSE_FIELDS.DATA]) {
      aboutInfo.value = response[RESPONSE_FIELDS.DATA]
      // Also set current version from about info
      currentVersion.value = response[RESPONSE_FIELDS.DATA].version || '1.0.0'
    }
  } catch (error) {
...
const handleClearCache = async (type) => {
  ...
  try {
    ...
    if (response[RESPONSE_FIELDS.SUCCESS]) {
      ElMessage.success(response[RESPONSE_FIELDS.MESSAGE] || '缓存已清理')
      // 发送缓存清理事件，通知其他组件刷新
      emitCacheCleared(type)
      // 刷新缓存信息
      await loadCacheInfo()
    } else {
      ElMessage.error(response[RESPONSE_FIELDS.ERROR] || '清理缓存失败')
    }
  } catch (error) {
