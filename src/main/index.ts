...
log.transports.console.level =
  process.env.NODE_ENV === "development" ? "info" : "debug";
...
async function initializeServices(): Promise<void> {
  ...
  log.info("[Main] Video proxy service initialized");

  // 8. HLS 代理服务
  hlsProxyService = new HlsProxyService();
  await hlsProxyService.initialize();
  log.info("[Main] HLS proxy service initialized");
