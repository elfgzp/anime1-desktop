...
export type GetAnimeListWithProgressResponse = APIResponse<
  import("./anime").AnimePageWithProgress
>;
...
export type GetBatchPlaybackProgressResponse = APIResponse<
  Record<string, PlaybackHistory | null>
>;
...
export type PreviewAutoDownloadFilterResponse =
  APIResponse<PreviewAutoDownloadFilterResponseData>;
...
export type RunAutoDownloadCheckResponse =
  APIResponse<RunAutoDownloadCheckResponseData>;
...
/**
 * 获取 HLS 代理 URL
 * IPC: anime:getHlsProxyUrl
 */
export interface GetHlsProxyUrlRequest {
  episodeUrl: string;
  videoUrl?: string;
}

export interface GetHlsProxyUrlResponse extends APIResponse<{
  url: string;
  isM3u8: boolean;
}> {}
